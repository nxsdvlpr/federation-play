...
export async function getContacts(query?: any): Promise<Contact[]> {
  await fakeNetwork(`get-contacts:${query}`);
...
export async function createContact(): Promise<Contact> {
  await fakeNetwork('create-contact');
...
export async function updateContact(id: string, updates: any): Promise<Contact> {
  await fakeNetwork(`update-contact:${id}`);
  const contacts: Contact[] | null = await localforage.getItem('contacts');
